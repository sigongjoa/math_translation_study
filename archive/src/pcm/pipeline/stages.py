...
class Stage2_Drafting(BaseStage):
    ...
    def _enforce_korean(result: str, original: str) -> str:
        ...
        if chinese / alpha > 0.2:
            return ""
        # If Korean chars are < 10% of alpha and result is non-trivial → not translated
        # Threshold lowered to 5 words to catch short untranslated English outputs
        if korean / alpha < 0.10 and len(result.split()) > 5:
            return ""
        return result
    ...
    def run(self, job, stage, sections_text, db, logger):
        ...
        try:
            ...
            for sec in processed:
                # Extract from Korean translation AND original English for robustness.
                # Korean translation is inconsistent (qwen2.5 mixes 군/그룹/Group);
                # original English is always reliable.
                ko_concepts = extract_concepts_from_text(sec.get("final_ko", ""))
                en_concepts = extract_concepts_from_text(sec.get("text", ""))
                concepts = list(set(ko_concepts) | set(en_concepts))
