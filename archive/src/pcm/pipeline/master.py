...
class MasterPipeline:
    ...
    def run(
        ...
        if not isinstance(start_page, int) or not isinstance(end_page, int):
            raise TypeError("start_page and end_page must be integers")
        if start_page < 0:
    ...
    def _parse_pdf(
        ...
        try:
            ...
            # Chunk any section that is too large for the LLM context window.
            # qwen2.5:7b handles ~400 words reliably; beyond that quality drops.
            sections = self._chunk_sections(sections, max_words=400)
    ...
    @staticmethod
    def _chunk_sections(sections: list[dict], max_words: int = 400) -> list[dict]:
        """Split any section whose word count exceeds max_words into smaller chunks.

        Splitting is done on paragraph boundaries (double-newline or ". " sentence
        endings) so that each chunk is a coherent unit for the LLM translator.
        """
        result = []
        for sec in sections:
            text = sec.get("text", "")
            ...
            if len(paragraphs) <= 1:
                # No paragraph breaks — fall back to sentence-level splitting
                import re
            ...
            for i, chunk_text in enumerate(chunks):
                chunk_id = f"{sec_id}_chunk{i+1}" if len(chunks) > 1 else sec_id
                result.append({**sec, "section_id": chunk_id, "text": chunk_text})
