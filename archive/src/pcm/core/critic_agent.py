...
class CriticAgent:
    ...
    def critique(self, original_en: str, translated_ko: str, glossary: dict) -> str:
...
Output your critique in Korean."""
        ...
        return self._client.chat_or_fallback(
            messages=[
                {"role": "system", "content": "You are a perfectionist math translator critic."},
                {"role": "user", "content": prompt},
            ],
            fallback="Critique failed.",
            temperature=0.1,
            timeout=120,
        )

    def refine(self, original_en: str, translated_ko: str, critique: str, glossary: dict) -> str:
...
4. Final output should be only the refined Korean text."""
        ...
        return self._client.chat_or_fallback(
            messages=[
                {"role": "system", "content": "You are an expert editor refining a math textbook."},
                {"role": "user", "content": prompt},
            ],
            fallback=translated_ko,
            temperature=0.1,
            timeout=120,
        )
