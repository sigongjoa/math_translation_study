...
이 책에서는 과학, 공학, 그리고 무역 등 다양한 분야에서 관찰되는 '구성성(compositionality)'을 탐구합니다. 구성성은 시스템이나 관계를 결합하여 새로운 시스템 또는 관계를 형성하는 것을 의미합니다. 각 사례에서 순수 수학에서 개발된 카테고리 이론적 구조가 해당 구성성을 아름답게 설명합니다. 이 장은 책의 첫 번째 장으로서 두 가지 역할을 수행해야 합니다. 첫째, 구성성에 대한 동기 부여를 위한 예시와 관련된 카테고리 이론적 공식화를 제공해야 합니다. 둘째, 책의 나머지 부분에 대한 수학적 기초를 마련해야 합니다. 독자의 배경 지식에 대한 최소한의 가정을 시작으로, 점진적으로 발전시켜야 하므로 초기 장의 예시는 필수적으로 단순화되어 있습니다. 그러나 독자가 카테고리 이론이 모델링에 가져오는 구조적 접근 방식을 이미 파악하기 시작할 수 있기를 바랍니다.


\section{부분 순서의 생성 효과는 구성 요소들의 합을 넘어서는 새로운 의미를 창출하는 것을 나타냅니다.}
\vspace{-4pt}{\small\color{ctgray} 1.1 More than the sum of their parts}\par\medskip


우리는 이 첫 장을 현실 세계 구조가 많은 경우 조합적이지만 관찰 결과는 종종 그렇지 않다는 사실에서 시작합니다. 이유는 관찰이 본질적으로 "정보 손실"이기 때문입니다. 어떤 정보를 추출하려면 세부 사항을 버려야 하기 때문입니다. 예를 들어, 실수를 일정 정밀도로 반올림하여 저장합니다. 그러나 특정 시스템 작업에서 세부 사항이 실제로 관련된 경우 해당 작업의 관찰 결과는 예상과 다를 것입니다. 이는 반올림 오차 또는 소수점 자릿수의 근사 오차의 경우 명확하지만 수치적 영역을 넘어서도 나타납니다. 복잡한 시스템을 관찰하는 것은 그 시스템의 동작을 예측하기에 충분하지 않기 때문입니다. 카테고리 이론의 핵심은 구조와 구조를 보존하는 함수들의 연구입니다. f : X → Y 는 X라는 개체에 대한 Y라는 다른 개체와 특정 관계를 통해 관찰하는 유형입니다. 예를 들어, X를 실험의 대상으로 생각해보세요.
