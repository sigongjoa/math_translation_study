...
제1장 생성 효과: 순서와 갈로아 접속

이 책에서는 과학, 공학, 그리고 상업 세계에서 관찰할 수 있는 '구성성(compositionality)' 이라는 개념을 탐구합니다. 구성성은 시스템이나 관계를 결합하여 새로운 시스템 또는 관계를 형성하는 것을 의미합니다. 각 경우에 우리는 순수 수학에서 사용되도록 개발된 카테고리 이론적 구조—이러한 상황의 구성성을 아름답게 설명합니다.—를 발견합니다. 이 장은 책의 첫 번째 장으로서 두 가지 역할을 수행해야 합니다. 첫째, 구성성에 대한 동기 부여적인 예시와 관련된 카테고리적 공식화를 제공해야 합니다. 둘째, 책의 나머지 부분에 대한 수학적 기반을 마련해야 합니다. 독자의 배경 지식에 대해 최소한의 가정으로 시작하기 때문에, 이 책은 점진적으로 진행되어야 하며, 초기 장의 예시는 불필요하게 단순화됩니다. 그러나 독자가 카테고리 이론이 모델링에 가져오는 구조적 접근 방식을 이미 파악할 수 있기를 바랍니다.


\section{1.1 부분들의 합을 넘어서는 것 (More than the sum of their parts)}
\vspace{-4pt}{\small\color{ctgray} 1.1 More than the sum of their parts}\par\medskip


우리는 이 첫 장을 현실 세계 구조들이 구성적이지만 관찰 결과는 종종 그렇지 않다는 사실에서 시작합니다. 이유는 관찰이 본질적으로 "손실"이기 때문입니다. 어떤 정보를 추출하려면 세부 사항을 버려야 하기 때문입니다. 예를 들어, 실수를 일정 정밀도로 반올림하여 저장합니다. 그러나 특정 시스템 작업에서 세부 사항이 실제로 중요한 경우, 해당 작업의 관찰 결과는 예상과 다를 것입니다. 이것은 반올림 오차의 경우 명확하지만, 수치적 영역을 넘어서도 나타납니다. 복잡한 시스템을 관찰하는 것은 그 시스템의 동작을 예측하기에 충분하지 않기 때문입니다. 카테고리 이론의 중심 주제는 구조와 구조를 보존하는 **상호 작용** 의 연구입니다.  **관찰 f : X → Y 는 특정 관계를 통해 객체 X를 객체 Y 에 상호 작용하는 방식으로 나타냅니다.** 예를 들어, X 를 실험 대상으로 생각하고...
