...
이 책에서는 과학, 공학, 그리고 무역 등 다양한 분야에서 관찰되는 구성성(compositionality)을 탐구합니다. 이는 시스템이나 관계를 결합하여 새로운 시스템 또는 관계를 형성하는 경우들을 말합니다. 각각의 사례에서 순수 수학에서 개발된 카테고리 이론적 구조가 해당 상황의 구성성을 아름답게 설명합니다. 이 장은 책의 첫 번째 장으로서 두 가지 역할을 수행해야 합니다. 첫째, 구성성에 대한 동기 부여를 위한 예시와 관련된 카테고리 이론적 공식화를 제공해야 합니다. 둘째, 책의 나머지 부분에 대한 수학적 기초를 마련해야 합니다. 독자의 배경 지식에 대한 최소한의 가정을 시작으로, 점진적으로 발전시켜야 하므로 초기 장의 예시는 불필요하게 단순화되어 있습니다. 그러나 독자가 카테고리 이론이 모델링에 가져오는 구조적 접근 방식을 이미 파악하기 시작할 수 있기를 바랍니다.


\section{\#\# 1.1  더 큰 합

이산(discrete) 구조의 조합은 단순히 구성 요소들의 합보다 더 큰 것을 생성하는 '생성 효과(generative effect)'를 보여줍니다. 이는 포셋(poset)에서 관찰되는 현상으로, 부분 순서 관계(partial order)와 골격성(skeletality)이 복잡한 구조를 만들어냅니다.}
\vspace{-4pt}{\small\color{ctgray} 1.1 More than the sum of their parts}\par\medskip


이 첫 번째 장을 다음과 같은 이유로 제시합니다: 현실 세계의 많은 구조들이 구성적이지만, 관찰 결과는 종종 그렇지 않다는 점입니다. 이는 관찰 자체가 "손실"이라는 본질적인 특징을 가지고 있기 때문입니다. 정보를 어떤 것에서 추출하려면 세부 사항을 버려야 하기 때문입니다. 예를 들어, 실수를 저장할 때는 일정 정밀도로 반올림합니다. 그러나 특정 시스템 작동에서 세부 사항이 실제로 중요한 역할을 한다면, 그 작동의 관찰 결과는 예상과 다를 수 있습니다. 이는 반올림 오차의 경우 명확하게 드러납니다. 하지만 수치적 영역 뿐만 아니라 복잡한 시스템을 관찰하는 것조차도 거의 항상 그 시스템의 동작을 예측하기에 충분하지 않습니다.  왜냐하면 관찰은 손실이라는 점 때문입니다. 카테고리 이론의 중심 주제는 구조와 구조를 보존하는 함수(map)의 연구입니다. 함수 f : X → Y 는 특정 관계를 통해 객체 X 를 객체 Y 에 관하여 관찰하는 방식입니다. 예를 들어, X 를 실험 대상으로 생각하고 1
